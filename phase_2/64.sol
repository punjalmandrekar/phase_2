...
Category: Accounting Inconsistency / Unchecked External Call
...
Description:
The dangerousSend() function performs a low-level ETH transfer using call() but completely ignores the returned success value.
...
If the receiver rejects ETH, the transfer fails and returns:
success = false
However, execution continues and updates protocol accounting:
totalSent += msg.value;
This causes internal state to indicate ETH was sent even though the transfer never occurred.

Impact:
Incorrect accounting.
State divergence from reality.
False reporting of transferred funds.
Potential downstream protocol logic failures.
...
Root Cause:
Failure to validate the return value of a low-level external call.

Vulnerable code:
_receiver.call{
    value: msg.value
}("");
The returned success boolean is discarded.

Recommendation:
Always verify the return value of low-level calls before updating protocol state.
...
Only update accounting after successful transfer.
*/

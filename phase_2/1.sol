...
Title: 
Missing Access Control in storeNumber()

Severity: 
Medium

Reason: 
Unauthorized users can modify persistent blockchain storage.
...
The storeNumber() function allows any external user to update the number state variable because no authorization mechanism is implemented.
The function is declared public and directly writes to storage without validating the caller identity.

Impact: 
An attacker can overwrite the stored value with arbitrary data.
If this variable controlled sensitive protocol logic such as: 
token pricing
treasury values
protocol configuration
governance parameters
then attackers could manipulate the protocol behavior and potentially cause financial or operational damage.
...
Root Cause:
The function is declared public without implementing access control.
No authorization check validates whether the caller is permitted to modify storage.

Recommendation:
Restrict write access using an owner-based authorization mechanism.

Example:
require(msg.sender == owner, "Not owner");
Initialize the owner inside the constructor for better security and readability.
*/

...
Reason:
Deleting array elements using delete array[index] creates empty gaps and inconsistent array state.
...
The deleteArrayIndex() function removes an array element using:
delete numbers[_index];
This operation does not shrink the array length.
Instead, the targeted index is reset to its default value while the array size remains unchanged.
This creates sparse arrays containing empty slots.

Impact:
Sparse arrays may introduce:
broken iteration logic
incorrect accounting
unexpected zero values
inefficient storage handling
application logic inconsistencies
If loops assume all array values are valid, empty slots may cause incorrect calculations or unexpected behavior.
...
3
Empty slot remains inside the array.

Root Cause:
The contract uses Solidity delete on a specific array index:
delete numbers[_index];
delete resets the element value but does not remove the element or reduce array length.

Recommendation:
Use the swap-and-pop pattern for efficient array removal.

Recommended approach:
Replace removed index with last element
Remove final array element using pop()
This prevents sparse arrays and keeps storage compact.
*/

//Patched Code:

contract DeleteStorageVariableFixed {

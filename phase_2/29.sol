...
Title:
Attempt to Modify Immutable Calldata

Severity:
Low

Reason:
The contract attempts to modify calldata directly, but calldata is read-only in Solidity.
...
The function tries to modify a calldata array directly:
_numbers[0] = 999;
But the array is declared as:
uint256[] calldata _numbers
Calldata is immutable and cannot be modified.
This causes the Solidity compiler to throw an error.

Impact:
The contract cannot compile successfully.
If developers misunderstand calldata behavior in real protocols, it may lead to:
broken logic
...
Proof of Concept:
Uncomment the vulnerable function:
function failModification(
    uint256[] calldata _numbers
)
    external
...
Compiler returns an error similar to:

TypeError:
Calldata arrays are read-only.

Root Cause:
The contract attempts to modify immutable calldata directly.
Solidity enforces calldata as read-only external input data.

Recommendation:
Copy calldata into memory before modification.

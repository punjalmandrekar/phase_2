...
Title:
Improper Validation Order Leading to Gas Inefficiency
...
Although Ethereum automatically reverts state changes on failure, updating storage before validation wastes gas and violates recommended Solidity security practices.
...
The brokenExecution() function modifies storage before checking whether _amount is valid.
...
If _amount > 5:
storage writes execute first
gas is consumed
require() fails afterward
EVM reverts the transaction
Even though storage changes disappear, unnecessary computation and gas usage still occur.

Impact:
The vulnerability does NOT permanently corrupt state because Ethereum transactions are atomic.
...
In larger protocols, repeated failed transactions may create scalability and operational issues.
...
false
Transaction Reverts
EVM rollback restores original state:
globalCounter = 3
balances[Alice] = 3
Temporary updates disappear completely.

Root Cause:
The contract performs state changes before input validation.

Problematic order:
1. Update storage
2. Validate input
This violates the recommended Checks-Effects-Interactions (CEI) pattern.

Recommendation:
Always validate inputs BEFORE modifying storage.

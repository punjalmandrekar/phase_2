...
Title: 
Memory vs Storage Reference Confusion

Severity: 
Medium

Reason: 
Improper understanding of storage references may lead to unintended persistent state mutations.
...
Any modification through this reference immediately updates persistent blockchain state.
Developers sometimes incorrectly assume the variable behaves like a temporary copy in memory, which may cause unintended state mutations and protocol corruption.

Impact:
Incorrect handling of storage references may unintentionally modify critical protocol state.

If similar logic controlled:
staking balances
treasury accounting
governance permissions
NFT ownership
reward calculations
then unintended mutations could corrupt protocol behavior.
Because storage persists permanently, accidental mutations may remain across all future transactions.
...
Result:
99, true
The storage reference directly modified blockchain storage.
...
Result:
99, false
The struct stored inside the mapping was permanently updated.

Root Cause:
The contract intentionally creates storage references using:
User storage user = users[msg.sender];
Storage references point directly to blockchain storage instead of creating temporary copies.
Any mutation through the reference immediately changes persistent state.

Recommendation:
Use memory when temporary copies are intended.

Example:
User memory user = users[msg.sender];
This creates isolated temporary data that does not modify blockchain storage.
Developers should clearly distinguish:
storage → persistent reference
memory → temporary copy
Auditors should carefully inspect reference-type behavior to prevent unintended state corruption.
*/
...
contract StorageReference {
    ...
    function testMemoryCopy(uint256 _newAge)
        public
        view
        returns (uint256, bool)
    {

...
Title: 
Missing Access Control in updateValue() and increaseValue()

Severity: 
Medium

Reason: 
Unauthorized users can mutate critical protocol state.
...
The functions updateValue() and increaseValue() allow any external user to modify the value state variable because no authorization mechanism is implemented.
Any user can directly overwrite or increase the stored value.

Impact:
An attacker can manipulate contract state with arbitrary values.
...
then unauthorized mutations could corrupt protocol behavior and financial logic.
...
Final value:
5100
Unauthorized state mutation succeeds.

Root Cause:
The functions are declared public without access restrictions.
No authorization validation exists such as:
require(msg.sender == owner)

Recommendation:
Restrict state mutation functions to trusted users only.

Use:
owner-based access control
modifiers
event logging for state transitions
Also track previous values for auditing and monitoring.

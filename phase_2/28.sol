...
Title:
Missing Input Validation for External Calldata Values

Severity:
Low

Reason:
The contract reads external calldata inputs directly without validation. Malicious or unexpected values may affect protocol logic.

Location:
Contract:
ReadCalldataValuesVul
...
The contract accepts external calldata inputs such as:
uint256 _number
string calldata _message
address _user

These values are fully controlled by users.
The contract directly reads and stores inputs without validation:
lastNumber = _number;
lastMessage = _message;
No checks exist for:
invalid values
...
This may create unexpected behavior in real protocols.

Impact:
Unvalidated calldata inputs may cause:
...
malicious inputs may affect application behavior.
...
Empty message stored successfully.
...
Zero address accepted without validation.

Root Cause

The contract trusts external calldata inputs directly:
...
No validation checks are performed before using or storing the values.

Recommendation

Validate external calldata inputs before processing.

...
Title:
Large Memory String Input May Cause High Gas Usage

Severity:
Low

Reason:
Using large memory strings can increase gas usage and may cause transaction failure due to excessive memory allocation.
...
The contract uses memory strings such as:
string memory _name
Memory strings are temporary and exist only during function execution.

If a very large string is passed:
more memory is allocated
gas usage increases
transaction may fail
Developers may not properly limit string size, which can create performance and gas issues.

Impact:
Large string inputs may lead to:
...
messaging systems
profile systems
on-chain storage systems
then excessive input sizes may affect protocol performance.

Proof of Concept:
Call:
saveName("VeryLargeString...")
...
Another example:
Call:
getWelcomeMessage("HugeString...")

Observe:
Memory usage increases during execution.

Root Cause:
The contract accepts dynamic memory strings without size validation:
string memory _name
Large dynamic inputs require additional memory allocation and gas.

Recommendation:
Validate or limit string size before processing.
Use calldata for external function inputs when modification is not required.

Example:
function saveName(string calldata _name) external
...
contract MemoryStringExample {
    ...
    */
        ...
        returns (string memory)
    {
        ...
        /*
            COPY TO MEMORY ONLY WHEN NEEDED
        */
        string memory message = _name;
